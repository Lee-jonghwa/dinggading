...
public class BandRecruitmentController implements RecruitmentApi {
    ...
    /**
     * POST /bands/{bandId}/recruitments : 밴드 구인 공고 등록
     * 밴드에 새로운 구인 공고를 등록합니다. 제목, 설명, 오디션 날짜, 오디션 곡 등 포함. 밴드는 동시간대에 하나의 구인 공고만 가질 수 있습니다.
     *
     * @param bandId                       밴드 ID (required)
     * @param createBandRecruitmentRequest (required)
     * @return 구인 공고 정보 (status code 201)
     * or 잘못된 요청입니다. (status code 400)
     * or 접근 권한이 없습니다. (status code 403)
     * or 요청한 리소스를 찾을 수 없습니다. (status code 404)
     */
    @Override
    public ResponseEntity<?> createBandRecruitment(Long bandId, CreateBandRecruitmentRequest createBandRecruitmentRequest) {
        log.info("Creating band recruitment for bandId: {}", bandId);

        UUID memberId = securityUtil.getCurrentMemberId();
        RecruitmentDTO recruitmentDTO = recruitmentService.createBandRecruitment(bandId, createBandRecruitmentRequest, memberId);
    ...
    /**
     * GET /bands/recruitments : 전체 구인 공고 목록 조회
     * 전체 구인 공고 목록을 조회합니다. 페이징 처리된 결과를 반환합니다.
     *
     * @param pageable
    ...
    public ResponseEntity<?> getAllRecruitments(Pageable pageable) {
        ...
        log.info("Retrieved {} recruitments, total elements: {}", recruitmentPage.getContent().size(), recruitmentPage.getTotalElements());
        return ResponseEntity.ok(recruitmentPage);
    }

    /**
     * PATCH /bands/{bandId}/recruitments/{recruitmentId} : 밴드 구인 공고 수정
     * 특정 구인 공고 정보를 수정합니다.
     *
     * @param bandId                       밴드 ID (required)
     * @param recruitmentId                구인 공고 ID (required)
     * @param createBandRecruitmentRequest (required)
     * @return 구인 공고 정보 (status code 200)
     * or 잘못된 요청입니다. (status code 400)
     * or 접근 권한이 없습니다. (status code 403)
     * or 요청한 리소스를 찾을 수 없습니다. (status code 404)
     */
    @Override
    public ResponseEntity<?> updateBandRecruitment(Long bandId, Long recruitmentId, CreateBandRecruitmentRequest createBandRecruitmentRequest) {
        log.info("Updating band recruitment. BandId: {}, RecruitmentId: {}", bandId, recruitmentId);

        UUID memberId = securityUtil.getCurrentMemberId();
        RecruitmentDTO recruitmentDTO = recruitmentService.updateBandRecruitment(bandId, recruitmentId, createBandRecruitmentRequest, memberId);
    ...
    /**
     * GET /bands/{bandId}/recruitments : 밴드 구인 공고 조회
     * 밴드의 구인 공고를 조회합니다. 밴드는 동시간대에 하나의 구인 공고만 가질 수 있습니다.
     *
     * @param bandId 밴드 ID (required)
     * @return 구인 공고 정보 (status code 200)
     * or 요청한 리소스를 찾을 수 없습니다. (status code 404)
    ...
    public ResponseEntity<?> getBandRecruitment(Long bandId) {
        ...
        log.info("Found recruitment for band ID: {}, recruitment ID: {}", bandId, recruitmentDTO.getBandRecruitmentId());
        return ResponseEntity.ok(recruitmentDTO);
    }

    /**
     * GET /bands/{bandId}/recruitments/{recruitmentId}/instruments : 구인 공고 모집 악기 목록 조회
     * 구인 공고의 모집 악기 포지션 정보를 조회합니다. 각 포지션별 요구 티어, 모집 인원 포함
     *
     * @param bandId        밴드 ID (required)
     * @param recruitmentId 구인 공고 ID (required)
     * @return 모집 악기 목록 (status code 200)
     * or 요청한 리소스를 찾을 수 없습니다. (status code 404)
     */
    ...
    public ResponseEntity<?> getRecruitmentInstruments(Long bandId, Long recruitmentId) {
        ...
        List<RecruitmentInstrumentDTO> instruments = recruitmentService.getRecruitmentInstruments(bandId, recruitmentId);
    ...
    /**
     * POST /bands/{bandId}/recruitments/{recruitmentId}/{recruitmentStatus} : 밴드 구인 공고 상태 변경
     * 밴드 구인 프로세스 상태를 변경합니다 (모집 중, 모집 완료)
     *
     * @param bandId            밴드 ID (required)
     * @param recruitmentId     구인 공고 ID (required)
     ...
     */
    @Override
    public ResponseEntity<?> updateRecruitmentStatus(Long bandId, Long recruitmentId, RecruitmentStatus recruitmentStatus) {
        ...
        RecruitmentDTO recruitmentDTO = recruitmentService.updateRecruitmentStatus(bandId, recruitmentId, status, memberId);
    ...
    /**
     * DELETE /bands/{bandId}/recruitments/{recruitmentId} : 밴드 구인 공고 삭제
     * 특정 구인 공고를 삭제합니다.
     *
     * @param bandId        밴드 ID (required)
     * @param recruitmentId 구인 공고 ID (required)
    ...
    /**
     * POST /bands/{bandId}/recruitments/{recruitmentId}/accept/{applicantId} : 지원자 승인
     * 특정 지원자의 밴드 가입을 승인합니다.
     *
     * @param bandId        밴드 ID (required)
     * @param recruitmentId 구인 공고 ID (required)
     * @param applicantId   지원자 ID (required)
     * @return 성공적으로 승인되었습니다. (status code 200)
     * or 잘못된 요청입니다. (status code 400)
     * or 접근 권한이 없습니다. (status code 403)
     * or 요청한 리소스를 찾을 수 없습니다. (status code 404)
     */
    ...
    /**
     * POST /bands/{bandId}/recruitments/{recruitmentId}/instruments : 구인 공고 악기 포지션 추가
     * 구인 공고에 새로운 악기 포지션을 추가합니다.
     *
     * @param bandId                          밴드 ID (required)
     * @param recruitmentId                   구인 공고 ID (required)
     * @param addRecruitmentInstrumentRequest (required)
     * @return 모집 악기 정보 (status code 201)
     * or 잘못된 요청입니다. (status code 400)
     * or 접근 권한이 없습니다. (status code 403)
     * or 요청한 리소스를 찾을 수 없습니다. (status code 404)
     */
    @Override
    public ResponseEntity<?> addRecruitmentInstrument(Long bandId, Long recruitmentId, AddRecruitmentInstrumentRequest addRecruitmentInstrumentRequest) {
    ...
    /**
     * POST /recruitments/{recruitmentId}/apply : 구인 공고 지원
     * 구인 공고에 지원합니다. 지원자 ID, 악기 포지션 포함
     *
     * @param recruitmentId           구인 공고 ID (required)
     * @param applyRecruitmentRequest (required)
     * @return 지원자 정보 (status code 201)
     * or 잘못된 요청입니다. (status code 400)
     * or 요청한 리소스를 찾을 수 없습니다. (status code 404)
     */
    @Override
    ...
    /**
     * DELETE /recruitments/{recruitmentId}/applicants/{applicantId} : 구인 공고 지원 취소
     * 구인 공고 지원을 취소합니다.
     *
     * @param recruitmentId 구인 공고 ID (required)
     * @param applicantId   지원자 ID (required)
    ...
    /**
     * GET /bands/{bandId}/recruitments/{recruitmentId}/applicants : 구인 공고 지원자 목록 조회
     * 구인 공고에 지원한 지원자 목록을 조회합니다.
     *
     * @param bandId        밴드 ID (required)
     * @param recruitmentId 구인 공고 ID (required)
     * @return 지원자 목록 (status code 200)
     * or 접근 권한이 없습니다. (status code 403)
     * or 요청한 리소스를 찾을 수 없습니다. (status code 404)
     */
    @Override
    ...
    /**
     * POST /bands/{bandId}/recruitments/{recruitmentId}/reject/{applicantId} : 지원자 거절
     * 특정 지원자의 밴드 가입을 거절합니다.
     *
     * @param bandId        밴드 ID (required)
     * @param recruitmentId 구인 공고 ID (required)
     * @param applicantId   지원자 ID (required)
     * @return 성공적으로 거절되었습니다. (status code 200)
     * or 접근 권한이 없습니다. (status code 403)
     * or 요청한 리소스를 찾을 수 없습니다. (status code 404)
     */
    @Override
    ...
    /**
     * DELETE /bands/{bandId}/recruitments/{recruitmentId}/instruments/{instrumentId} : 구인 공고 악기 포지션 제거
     * 구인 공고에서 특정 악기 포지션을 제거합니다.
     *
     * @param bandId        밴드 ID (required)
     * @param recruitmentId 구인 공고 ID (required)
