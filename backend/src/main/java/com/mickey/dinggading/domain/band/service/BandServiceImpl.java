...
public class BandServiceImpl implements BandService {
    ...
    public PageBandDTO getAllBands(Pageable pageable) {
        ...
        List<BandDTO> bandDTOs = bandPage.getContent().stream()
            .map(band -> {
                // 해당 밴드의 멤버 가져오기
                List<BandMember> bandMembers = bandMembersMap.getOrDefault(band.getBandId(), Collections.emptyList());
    ...
    @Override
    public BandMemberDTO updateBandMemberInstrument(Long bandId, UUID memberId, Instrument instrument, UUID requesterId) {
        // 밴드 존재 여부 확인
        Band band = getBand(bandId);
